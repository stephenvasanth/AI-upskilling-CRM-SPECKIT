...
| AUTH-02 | JWT tokens expire after 8 hours; the user is redirected to login on expiry or any 401/403 response | Must Have |
...
| NFR-P03 | All data fetches MUST go to Redis cache first; on cache miss, fetch from PostgreSQL and populate cache; TTL = 24 hours; on any create/update/delete, the relevant cache key(s) MUST be invalidated immediately (see `crm-service/rules.md` Caching Strategy) |
...
| ID | Requirement |
|----|------------|
| NFR-S01 | All API endpoints except `/api/auth/login` require a valid JWT; tokens expire after 8 hours; the client redirects to `/login` on expiry or any 401/403 response |
...
| Layer | Technology | Project | Notes |
|-------|-----------|---------|-------|
| Frontend | Angular 20 | `crm-ui/` | SPA; desktop browsers only (min 1280 px) |
| Backend | Java 21 + Spring Boot 3.3.x | `crm-service/` | REST API; Java 21 LTS (supported until 2031) |
| Database | PostgreSQL | `crm-service/` | Primary data store; idempotent migrations on startup |
| Cache | Redis | `crm-service/` | Cache-first; TTL 24 h; invalidated on every write |

> **Java setup note**: JDK 21 is available at `JAVA_HOME`. Configure Maven/Gradle
> to use `JAVA_HOME` rather than the system `java` command (currently Java 8).
> Spring Boot 3.3.x requires Java 17+.

### 4.6 Project Structure

Both projects live as sibling directories at the repository root:

```
AI-CRMusing-speckit/
├── crm-ui/          # Angular 20 frontend (see crm-ui/rules.md)
├── crm-service/     # Spring Boot 3.3 backend (see crm-service/rules.md)
├── docs/            # Requirements, design, stitch.ai output
└── specs/           # Spec Kit feature specs and plans
```

Each project maintains its own `rules.md` file at its root defining coding
standards, architectural conventions, and quality gates specific to that
technology stack.

### 4.7 Architectural Rules

#### crm-service (Backend) Rules

| Rule | Requirement |
|------|------------|
| **Caching — cache-first** | Every read operation MUST check Redis before querying PostgreSQL |
| **Caching — populate on miss** | On cache miss, fetch from DB, write to Redis, then return result |
| **Caching — TTL** | All cache entries MUST use TTL = 24 hours |
| **Caching — invalidation** | On create, update, or delete, all affected cache keys MUST be invalidated immediately |
| **Layered architecture** | Controller → Service → Repository; no business logic in controllers or repositories |
| **Error responses** | Always return structured JSON `{ "error": { "code", "message", "fields?" } }`; never HTML |
| **JWT validity** | Tokens expire after 8 hours; secret loaded from environment variable |
| **Password storage** | BCrypt hash only; raw passwords never logged or stored |
| **No sensitive logging** | Tokens, passwords, and PII must never appear in any log level |
| **Migration strategy** | Flyway or Liquibase; scripts must be idempotent; run automatically on startup |
| **CORS** | Restricted to configured `FRONTEND_ORIGIN` environment variable |
| **Input validation** | All request bodies validated with Bean Validation (`@Valid`) before reaching the service layer |

#### crm-ui (Frontend) Rules

| Rule | Requirement |
|------|------------|
| **Standalone components** | All Angular components MUST use `standalone: true` (Angular 17+ standard) |
| **Typed HTTP client** | All API calls use Angular `HttpClient` with typed response interfaces |
| **Design tokens** | All styles use CSS custom properties from `docs/DESIGN.md` §3; no hardcoded colour or spacing values |
| **Lazy loading** | Each module (contacts, deals, activities, tasks, admin) MUST be lazy-loaded via Angular Router |
| **JWT handling** | Token stored in `localStorage`; attached via HTTP interceptor; cleared on 401/403 |
| **Error handling** | All HTTP errors handled via a global Angular HTTP interceptor; user-facing messages via toast |
| **Form validation** | All forms use Angular Reactive Forms with typed `FormGroup`; inline validation before submit |
| **Bundle size** | Initial bundle MUST stay below 500 KB gzipped; enforce via `ng build --stats-json` |
| **Strict TypeScript** | `strict: true` in `tsconfig.json`; no `any` types in production code |
| **Accessibility** | All interactive elements keyboard-navigable; ARIA labels on icons; focus rings never hidden |
